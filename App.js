...
const getUsers = username =>
  axios
    .get(`https://user-resource-8453.twil.io/users`)
    .then(twilioUsers => twilioUsers.data);

const createTwilioUser = params =>
  axios
    .get(`https://user-resource-8453.twil.io/create?params=${params}`)
    .then(twilioUsers => twilioUsers.data);

const updateUser = params =>
  axios
    .get(
      `https://user-resource-8453.twil.io/update?identity=${
        params.identity
...
const App = _state => {
  ...
  console.log(_state, 'this is _state user ');
  ...
  React.useEffect(() => {
    const subscriptions = [
      ...
      twilioPhoneEmitter.addListener('CallDisconnected', data => {
        if (Platform.OS == 'ios') showAlert('Call Disconnected', 'long');
      }),
  ...
  useEffect(() => {
    const unsubscribe = messaging().onMessage(async remoteMessage => {
      ...
      var id = remoteMessage.data.sender;
      var senderNumber = id.substr(id.length - 10);
      console.log(senderNumber, 'last Number');
      dispatch(
        incomingMessages(
          {
            sid: remoteMessage.messageId,
            message: remoteMessage.data.body,
            createdAt: remoteMessage.notification.sentTime,
  ...
  return (
    <View style={{flex: 1}}>
      {loader && loader.isLoading ? (
        <SpinKitElement loader={loader || {}} />
      ) : null}
      {user && user.id ? <DrawerNavigator /> : <AnonymousNavigator />}
    </View>
  );
};
...
const enhance = compose(
  connect(mapStateToProps, {
    ...userAccountStoreActions,
    ...userAccountServiceActions,
    setLoader,
  }),
  ...
  lifecycle({
    async componentDidMount() {
      ...
      await self.props.getAccount(
        {userId: user.uid, background: true},
        function (result) {
          fetchAndUpdateFCMToken(function (token) {
            var params = null;
            if (token && self.props.stateAuth && self.props.stateAuth.user) {
              params = {
                userId: self.props.stateAuth.user.id,
                background: true,
                data: {
                  version: Config.APP_VERSION_NAME,
                  buildNumber: Config.APP_VERSION_CODE,
                },
              };
            }
            if (
              self.props.stateAuth &&
              self.props.stateAuth.FCMToken != token
            ) {
              params.data.notificationToken = token;
            }
            if (params) {
              self.props.updateAccount(
                params,
                function (result) {
...
//   // if (!global.userEmailId) return showAlert('Email Missing');
//   // const identity = global.userEmailId;
//   var phoneNumber = '2123851384';
//   // if (identity == 'nijaahnandhrv@digisenz.com') phoneNumber = '7598198955';
//   // else if (identity == 'gopim@digisenz.com') phoneNumber = '9195990852';
//   // else
//   //   showAlert(
//   //     'Couldnt find email so fetching token with default number. Check fetchAccessToken method in app.js',
//   //     null,
//   //     'top',
//   //   );
