...
const enhance = compose(
    ...
    withHandlers({
        onPlayRecordingPressed: ({ contactDetail, setContactDetail }) => async (audioProp, audioIndex) => {
            ...
            if (contactDetail.callHistory.length > 1) {
                _.each(contactDetail.callHistory, eachRec => {
                    if (eachRec.recordedVoice && (audioProp.recordedVoice != eachRec.recordedVoice)) {
            ...
            TrackPlayer.setupPlayer().then(async (setUpPlayerResp) => {
                ...
                TrackPlayer.addEventListener('playback-state', async (data) => {
                    console.log(data);
                    if (data.state == TrackPlayer.STATE_BUFFERING && global.audioTrackIndex == audioIndex && !contactDetail.callHistory[audioIndex].isBuffering) {
                    ...
                    } else if (data.state == TrackPlayer.STATE_LOADING && global.audioTrackIndex == audioIndex && !contactDetail.callHistory[audioIndex].isLoading) {
                        ...
                        contactDetail.callHistory[audioIndex].isStopped = false;
                        setContactDetail(contactDetail);
                    } else if (data.state == TrackPlayer.STATE_PLAYING && global.audioTrackIndex == audioIndex && !contactDetail.callHistory[audioIndex].isPlaying) {
                        contactDetail.callHistory[audioIndex].isBuffering = false;
                        contactDetail.callHistory[audioIndex].isLoading = false;
                        ...
                        contactDetail.callHistory[audioIndex].isStopped = false;
                        setContactDetail(contactDetail);
                    } else if (data.state == TrackPlayer.STATE_PAUSED && global.audioTrackIndex == audioIndex && !contactDetail.callHistory[audioIndex].isPaused && !contactDetail.callHistory[audioIndex].isStopped) {
                        ...
                        contactDetail.callHistory[audioIndex].isStopped = false;
                        setContactDetail(contactDetail);
                    } else if (data.state == TrackPlayer.STATE_STOPPED && global.audioTrackIndex == audioIndex && !contactDetail.callHistory[audioIndex].isStopped) {
                        contactDetail.callHistory[audioIndex].isPaused = false;
                        contactDetail.callHistory[audioIndex].isBuffering = false;
                        contactDetail.callHistory[audioIndex].isPlaying = false;
                        contactDetail.callHistory[audioIndex].isLoading = false;
                ...
                TrackPlayer.add({
                    ...
                    TrackPlayer.play().then(async (playResp) => {
                        ...
                        contactDetail.callHistory[audioIndex].isPaused = false;
                        contactDetail.callHistory[audioIndex].isBuffering = false;
                        contactDetail.callHistory[audioIndex].playingMin = position / 60;
                        contactDetail.callHistory[audioIndex].totalMin = duration / 60;
                        setContactDetail(contactDetail);
                    }, (error) => {
                        contactDetail.callHistory[audioIndex].isLoading = false;
                        setContactDetail(contactDetail);
        ...
        onStopPlayingPressed: ({ contactDetail, setContactDetail }) => async (audioProp, audioIndex) => {
            contactDetail.callHistory[audioIndex].isPlaying = false;
            contactDetail.callHistory[audioIndex].isLoading = false;
            contactDetail.callHistory[audioIndex].isPaused = false;
            contactDetail.callHistory[audioIndex].isBuffering = false;
...
=======
        onCallPressed: ({ contactDetail, user }) => async () => {
            ...
            RNTwilioPhone.startCall(contactDetail.phone).then((resp) => {
                console.log(resp);
            }, (err) => {
                console.log(err);
            });
>>>>>>> call issue fix
    ...
    lifecycle({
        async componentDidMount() {
            ...
            var activeCalls = await RNCallKeep.checkIfBusy();
            if (activeCalls)
                self.props.setIsCallActive(true);
            var contactDetail = self.props.route.params && self.props.route.params.contactProp ? self.props.route.params.contactProp : null;
